...
pub struct ImportProgress {
    ...
    /// 实时网速（仅下载步骤），如 "1.2MiB/s"
    pub speed: Option<String>,
    /// 字节级真实进度（仅下载步骤），如 "12.3MiB / 26.9MiB"
    pub detail: Option<String>,
}

const IMPORT_STEPS: i32 = 4;

pub(crate) async fn setting(pool: &SqlitePool, key: &str) -> Option<String> {
...
pub(crate) async fn ytdlp_config(pool: &SqlitePool) -> YtDlp {
    YtDlp {
...
/// 抓取视频元数据并插入 videos 行，返回 (行 id, youtube_id)
async fn fetch_and_insert(pool: &SqlitePool, yt: &YtDlp, url: &str) -> Result<(i64, String), String> {
...
pub async fn import_video(
    ...
    // 已完整导入则直接返回；video_path 为空说明上次下载中断/失败，复用记录直接续传
    let existing = sqlx::query_as::<_, Video>("SELECT * FROM videos WHERE url = ?")
        .bind(&url)
        .fetch_optional(pool.inner())
        .await
        .map_err(|e| e.to_string())?;

    let (video_id, yt_id) = match existing {
        Some(v) if v.video_path.is_some() => return Ok(v),
        Some(v) => match v.youtube_id.clone() {
            Some(y) => {
                // 断点续传：元数据已存库，跳过信息抓取；yt-dlp --continue 续传 .part
                emit(0, "已保存视频信息", Some(100.0));
                (v.id, y)
            }
            None => {
                // 元数据不全的残缺记录没法续传，删掉重来
                sqlx::query("DELETE FROM videos WHERE id = ?")
                    ...
                    .map_err(|e| e.to_string())?;
                emit(0, "获取视频信息", None);
                fetch_and_insert(pool.inner(), &yt, &url).await?
            }
        },
        None => {
            emit(0, "获取视频信息", None);
    ...
    let (video_path, sub_path, thumb_path) = ingest::download(
        ...
        &media_dir,
        &yt,
        move |p: ingest::DlProgress| {
            let _ = app2.emit(
                "import-progress",
                ImportProgress {
                    step: 1,
                    total_steps: IMPORT_STEPS,
                    label: p.stage.to_string(),
                    percent: p.percent,
                    speed: p.speed,
                    detail: p.detail,
                },
    ...
    .await
    // 下载失败时保留 videos 行（video_path 为空）与 .part 文件，下次导入自动续传
    .map_err(|e| {
        let msg = e.to_string();
    ...
    let (subtitle_source, audio_path) = match sub_path {
        ...
        None => {
            ...
            let wav = media_dir.join(format!("{yt_id}.wav"));
...
pub async fn delete_video(
    ...
    // 清理缩略图/叠层缓存目录（app_data 下按视频 id 分目录）
    if let Some(data_dir) = media_dir.parent() {
        for sub in ["storyboard", "thumbs", "export_overlay"] {
            let _ = std::fs::remove_dir_all(data_dir.join(sub).join(id.to_string()));
        }
    }
    Ok(())
}
